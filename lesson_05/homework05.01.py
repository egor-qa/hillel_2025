...
# car_data.items() - це звернення до пари ключ:значення
# lambda - використовується для сортування словника, у якому значення - це кортеж
# key=lambda x: x[1][-1] - говорить що сортувати треба за останнім елементом кортежа
# x[1] - звернення до кортежу
# [-1] - звернення до елементу кортежа
# reverse=True - каже що сортувати треба за спаданням. якщо reverse не вказано (або False), то за зростанням
